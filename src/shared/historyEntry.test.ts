...
import { MAX_BODY_BYTES, mergeHistoryEntries, truncateBody } from './historyEntry.js';
...
describe('truncateBody', () => {
  ...
  it('truncates bodies larger than the cap', () => {
    ...
    expect(new TextEncoder().encode(result.body).length).toBeLessThanOrEqual(MAX_BODY_BYTES + 200);
  });
});
...
describe('mergeHistoryEntries', () => {
  ...
  it('places pending before existing and dedupes by id', () => {
    ...
    expect(mergeHistoryEntries(pending, existing).map((entry) => entry.id)).toEqual([
      'b',
      'a',
      'c'
    ]);
